...
void DX::DeviceResources::CreateDeviceResources() 
{
	...
	DX::ThrowIfFailed(m_d3dDevice->CreateDepthStencilState(&depthStencilDesc, &m_d3dDepthStencilState));

	depthStencilDesc.DepthEnable = false;
	depthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;

	DX::ThrowIfFailed(m_d3dDevice->CreateDepthStencilState(&depthStencilDesc, &m_d3dDepthStencilStateZDisable));

	m_d3dContext->OMSetDepthStencilState(m_d3dDepthStencilStateZDisable.Get(), 0);
	...
	DX::ThrowIfFailed(m_d3dDevice->CreateBlendState(&blendStateDescription, &m_d3dBlendStateAlfaEnabled));

	float blend[] = { 1.0f, 1.0f, 1.0f, 1.0f };
	m_d3dContext->OMSetBlendState(m_d3dBlendStateAlfaEnabled.Get(), blend, 1);

	blendStateDescription.RenderTarget[0].BlendEnable = FALSE;

	DX::ThrowIfFailed(m_d3dDevice->CreateBlendState(&blendStateDescription, &m_d3dBlendStateAlfaDisabled));
	...
	DX::ThrowIfFailed(m_d3dDevice->CreateRasterizerState(&rasterizerStateDescription, &m_d3dRasterizerState));
